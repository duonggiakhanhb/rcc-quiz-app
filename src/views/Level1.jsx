...
export default class Level1 extends Component {
  ...
  checkMatched = () => {
    ...
    if (
        (randomTense === "simple" ? level1[round].simple : level1[round].past) ===
        value
    ) {
      ...
      this.setState(
          ...
          () => {
            let subArray= [];
            subArray = subArray.concat([level1[round].voca, value, this.state.wrongAnswer]);
    ...
    return (
        ...
        >
          {round < level1.length ? (
              <>
                <h1>Quiz Game</h1>
                <Progress percent={(round / level1.length) * 100} status="active" />
              ...
              <>
                <h1>{`Score: ${(1 - wrongAnswers.length /level1.length) * 100}`}</h1>
